...
class PanyeroSupabaseUser extends BaseAuthUser {
  PanyeroSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> panyeroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PanyeroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
